...
{{
  ...
  "generated_title": "<SEO-optimised headline for this new AI capability. Rules: (1) 50-65 characters total. (2) Do NOT use any 'First Look:' prefix — the article badge and category handle that. (3) MUST include the vendor name (Google, OpenAI, Anthropic, Meta, Microsoft, AWS, NVIDIA, etc.) — this is a primary search term. (4) MUST include the product or model name if one is mentioned (e.g. Gemini 2.5, Claude Opus 4, GPT-5). (5) Describe what shipped in plain terms — not the security risk. (6) Use action verbs: Ships, Launches, Releases, Adds, Brings, Opens. (7) Do NOT start with 'How', 'Why', or 'What'. Good examples: 'Anthropic Ships Claude Code with Terminal Access for Coding Agents', 'Google Launches Gemini 2.5 with Agentic File Access', 'AWS Brings NVIDIA Nemotron to GovCloud'. Bad examples: 'First Look: Anthropic Ships Claude Code' (has forbidden prefix), 'A New AI Capability Has Arrived' (no vendor, no product), 'How Google Is Changing AI With Gemini' (starts with How).>",
