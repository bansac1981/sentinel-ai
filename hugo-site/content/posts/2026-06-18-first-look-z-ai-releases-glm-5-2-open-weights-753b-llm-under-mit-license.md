...
tldr_actions:
  - "Audit internal policies for self-hosted LLM deployment and add GLM-5.2 to approved/denied model registries"
  - "Update DLP and SIEM rules to detect large-volume inference traffic indicative of self-hosted frontier-model use on corporate infrastructure"
  - "Assess supply-chain risk from MIT-licenced derivative weights that may be fine-tuned to remove safety behaviours before downstream adoption"
